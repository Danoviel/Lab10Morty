...
async function getPokemon(name: string): Promise<Pokemon> {
  const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, {
    next: { revalidate: 86400 }, // ISR: revalida cada 24 horas
  });

  // Activa not-found.tsx
  if (!res.ok) notFound();

  return res.json();
}

// SSG: genera rutas estáticas para los 151 pokémon
  ...
  return (
    <div className="p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div
          className={`bg-linear-to-r from-black ${typeColors[pokemon.types[0].type.name] ?? "bg-gray-400"} p-8`}
        >
